...
(function (root, factory) {
  ...
  function stepParticles(state, dt) {
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      if (params.wrap) {
        // Keep wrapped fields from forming sticky edge bands after re-entry.
        var edgeBand = Math.max(24, Math.min(w, h) * 0.09);
        var edgeRelease = 2;
        if (p.x < edgeBand) ax += (1 - p.x / edgeBand) * edgeRelease;
        else if (p.x > w - edgeBand) ax -= (1 - (w - p.x) / edgeBand) * edgeRelease;
        if (p.y < edgeBand) ay += (1 - p.y / edgeBand) * edgeRelease;
        else if (p.y > h - edgeBand) ay -= (1 - (h - p.y) / edgeBand) * edgeRelease;
      }
