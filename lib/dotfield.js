...
(function (root, factory) {
  ...
  function wrapPosition(value, size, inset) {
    ...
    if (!inset) {
      if (value < 0) return Math.max(0, size + (value % size));
      if (value > size) return Math.min(size, value % size);
      return value;
    }

    // A non-zero inset is a seam-clearance zone. Wrap immediately when a
    // particle enters it so slow or tangential motion cannot hug the border.
    var min = inset;
    var max = size - inset;
    var span = max - min;
  ...
  function stepParticles(state, dt) {
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      if (params.wrap) {
        // Preserve velocity through the seam. A non-zero inset keeps clipped
        // particles from lingering in a visible lane along the canvas border.
